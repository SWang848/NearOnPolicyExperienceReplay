...
class ActorCritic(Critic):
    ...
    def choose_action(self, weight, observation, action_last_input, target=False, stochastic=True, use_prob=True, return_probs=False):
        if target:
            q_value, acts_prob = self.target_model.predict([weight, observation, action_last_input])
        else:
