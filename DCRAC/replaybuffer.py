...
class NearOnPolicyMemoryBuffer(PrioritizedDiverseMemory):
    def __init__(
            self,
            main_capacity,
            sec_capacity,
    ...
    def cal_weights_similarity(self, a, b, mode):
        if mode=='euclidean':
            dist = np.linalg.norm(a-b)
        elif mode=='cosine':
            dist = self.cosine_distance(a, b)

        return -dist
    
    def cal_states_similarity(self, state, mode):
        seed = 0
        ...
        if mode == 'euclidean':
            ...
            dist = self.cosine_distance(state[0, :], state[1, :])
        
        return -dist

    def cal_properties_similarity(self, properties_1, properties_2, mode):
        if mode == 'euclidean':
            dist = np.linalg.norm(properties_1-properties_2)
        elif mode == 'cosine':
            dist = self.cosine_distance(properties_1, properties_2)

        return -dist

    def sample(self, th, n, k, steps, current_weights, current_state, properties, property=False, mode='euclidean', tree_id=None):
        if n<1:
            return None, None, None
