...
hyper_info = '{}_{}-r{}{}-d{}-t{}-batsiz{}-{}steps-lr{}-lr2{}-{}-acteval_{}-Ner{}'.format(
    options.agent, options.net_type, options.replay, str(options.buffer_size), options.dup, options.timesteps, 
    options.batch_size, options.steps, str(options.lr_c), str(options.lr_a), options.mode, options.obj_func, str(options.ner))

# create evironment
env = DeepSeaTreasure(view=(5,5), full=False, scale=9) if options.dst_view == '5' else DeepSeaTreasure(view=(3,3), scale=15)
