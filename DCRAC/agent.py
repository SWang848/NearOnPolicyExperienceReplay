...
from replaybuffer import NearOnPolicyMemoryBuffer, MemoryBuffer
...
class DCRACAgent:
    ...
    def train(self, log_file, learning_steps, weights, 
        ...
        for step in range(self.total_steps):
            ...
            self.log.log_step(episodes, step, loss, reward,
            ...
            if terminal or episode_steps > self.max_episode_length:
                ...
                is_weight_change = int(
                    (step + 1) / per_weight_steps) != weight_index

                if per_weight_steps == 1:
                    weight_index += 1
                else:
                    weight_index = int((step + 1) / per_weight_steps)
    ...
    def memorize(self, state, properties, action, reward, next_state, terminal, action_prev, acts_prob, 
        ...
        extra = np.array([None, None, self.epsilon, self.weights, properties], dtype=object)
...
class DCRACSAgent(DCRACAgent):
    ...
    def pick_action(self, trace_o, trace_a, weights=None):
        ...
        action, acts_prob = self.actorcritic.choose_action(
            weights[np.newaxis, ], trace_o[np.newaxis, ], trace_a[np.newaxis, ], 
            stochastic=self.stoch_policy, use_prob=self.use_prob, return_probs=True)
